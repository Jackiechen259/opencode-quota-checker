...
unsafe fn public_window_callback_inner(
    ...
    let callback = || match msg {
        ...
        WM_DPICHANGED => {
            ...
            if dragging_window {
                // Windows 11 24H2 can report the old monitor from
                // `MonitorFromWindow` while a mixed-DPI drag is crossing the
                // display boundary. The monitor-nudge logic below then moves
                // the window back, triggers another WM_DPICHANGED, and grows
                // the window on every iteration. During an active system move,
                // use the rectangle Windows supplied for the new DPI directly.
                //
                // https://github.com/rust-windowing/winit/issues/4600
