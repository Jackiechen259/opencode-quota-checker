...
| Native edge resize of borderless window | winit WNDPROC subclass (`WM_NCHITTEST`) | Tauri `decorations:false` + Windows adapter (same technique) | ⬜ |
...
| Native notifications | Windows Toast / macOS osascript / Linux notify-send | `tauri-plugin-notification` (fallback native adapter kept) | ⬜ |
...
| Installer launch (NSIS / DMG / AppImage / deb) | custom `update/installer.rs` | `tauri-plugin-updater` install / fallback adapter | ⬜ |
| Release notes in browser | custom `open_url` | `tauri-plugin-opener` (Rust side) | ⬜ |
...
| Schema 1 & 2 configs load | `#[serde(default)]` migration (no silent loss) | ⬜ |
...
|---|---|---|---|
| Workspace | `crates/opencode-core`, `crates/opencode-desktop`, `xtask` | + `src-tauri`, eventually − desktop | ⬜ |
| Frontend build | — | Vite + React + TS, `pnpm build` | ⬜ |
| Windows packaging | cargo-packager NSIS (currentUser, EN + zh-CN) | Tauri NSIS (same modes/languages) | ⬜ |
